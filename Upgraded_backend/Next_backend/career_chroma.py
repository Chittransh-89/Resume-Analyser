"""Career data in ChromaDB - Production version"""

import chromadb

# ===========================================
# YOUR CAREER_PATHS dictionary (same as above)
# ===========================================
CAREER_PATHS = {
    # ── SOFTWARE & TECH ──
    "frontend_developer": {
...
# ===========================================
client = chromadb.PersistentClient(path="Career-assistant_v2/backend/databases/career_db")
collection = client.get_or_create_collection(name="careers")


# ===========================================
# BLOCK 2: Build Database (only if empty)
# ===========================================
if collection.count() == 0:
    print("📚 Building career database...")
    ...
    for career_id, career in CAREER_PATHS.items():
        ...
        # Skills ko text mein convert karo
        # Required skills ko flatten karo (nested dict ko string mein)
        skills_text = ""
        for skill_category, skills_list in career.get('required_skills', {}).items():
            skills_text += f"{skill_category}: {', '.join(skills_list)}. "
        
        # Roadmap ko text mein convert karo
        roadmap_text = ""
        for phase in career.get('roadmap', []):
            roadmap_text += f"{phase['phase']}: {', '.join(phase['tasks'])}. "
...
Salary in India: {career['salary_range']['india']}
Salary in US: {career['salary_range']['us']}
Salary Remote: {career['salary_range']['remote']}

Job Titles: {', '.join(career['job_titles'])}
Companies Hiring: {', '.join(career['companies_hiring'])}
...
"""
    ...
    print(f"✅ Added {len(documents)} careers!")
else:
    print(f"✅ Database already has {collection.count()} careers")
...
# Test 1: Web development search
print("\n🔍 Search: 'I want to learn web development'")
results = collection.query(
    query_texts=["I want to learn web development"],
    n_results=2
)
...
# Test 2: Creative search
print("\n🔍 Search: 'I am creative and love designing'")
results = collection.query(
    query_texts=["I am creative and love designing"],
    n_results=2
)
for i, doc in enumerate(results['documents'][0]):
    title = results['metadatas'][0][i]['title']
...
print("\n🔍 Search: 'easy career' (only Beginner Friendly)")
results = collection.query(
    query_texts=["easy career to start"],
    n_results=5,
    where={"difficulty": "Beginner Friendly"}
)
for i, doc in enumerate(results['documents'][0]):
    title = results['metadatas'][0][i]['title']
    print(f"  {i+1}. {title}")
...
# Test 4: Get specific career
print("\n📄 Get specific career: 'data_scientist'")
result = collection.get(ids=["data_scientist"])
print(f"  Title: {result['metadatas'][0]['title']}")
